...
string dirPath = @"J:\J12172_WORM_Stage_3B_(Water_Corporation)\Drawings and Documents\E80 Docs\Development\#054-MINDARIE TEMPORARY\Marked up drawings";
void Main()
{
...
private string[] getFilePaths()
{
	...
	try
	{
		string[] MultipleFilters = {"*tif","*TIF"};
		// Only get files that ending with the letter "tif" or "TIF".
		foreach(var filter in MultipleFilters)
		{
			dirs = Directory.GetFiles(dirPath, filter);	
		}
		
		Console.WriteLine("The number of files ending with tif is {0}.", dirs.Length);
